...
export default function Settings() {
  ...
  useEffect(() => {
    if (!socket || !user?.pseudo) return;

    const handleVolunteerUpdate = ({ pseudo, status }) => {
      if (pseudo !== user.pseudo) return;
      setVolunteerProfile(prev => prev ? { ...prev, status } : prev);
      setSuccess('Statut mis à jour en temps réel.');
    };

    socket.on('volunteer_update', handleVolunteerUpdate);
    return () => {
      socket.off('volunteer_update', handleVolunteerUpdate);
    };
  }, [socket, user?.pseudo]);
  ...
  const handleSaveProfile = async (e) => {
    ...
    setError('');
    setSuccess('');
    if (!form.university.trim() || !form.year.trim()) {
      setError('L’université et l’année sont requises.');
      setSaving(false);
      return;
    }
    try {
  ...
  const handleRegister = async (e) => {
    ...
    setError('');
    setSuccess('');
    if (!registerForm.university.trim() || !registerForm.year.trim()) {
      setError('L’université et l’année sont requises pour devenir volontaire.');
      setSaving(false);
      return;
    }
    e.preventDefault();
  ...
  const handleStatusChange = async (status) => {
    if (!volunteerProfile || volunteerProfile.status === status) return;
          ...
          {tab === 'volontaire' && isVolunteer && (
            <div className="settings__section">
              ...
              <div className="settings__status-section">
                ...
                <div className="settings__status-overview">
                  <span className={`settings__status-badge settings__status-badge--${volunteerProfile?.status}`}>
                    {STATUS_OPTIONS.find(opt => opt.value === volunteerProfile?.status)?.label || 'Hors ligne'}
                  </span>
                  <span className="settings__status-current">Statut actuel : <strong>{STATUS_OPTIONS.find(opt => opt.value === volunteerProfile?.status)?.label || 'Hors ligne'}</strong></span>
                </div>
